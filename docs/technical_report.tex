...
\hypersetup{
  ...
  pdftitle={Real or Fake? Transformation Robustness Does Not Guarantee Cross-Generator Generalization},
...
\begin{center}
  ...
  Guarantee Cross-Generator Generalization\par}
  \vspace{0.15in}
...
samples, the selected hybrid produced AUCs of 0.4975, 0.5025, and 0.5000 and called every image fake.
Its semantic-only component retained AUCs of 0.8870, 0.7680, and 0.7406. The largest absolute
standardized frequency feature was 12.15 on clean CIFAKE but about 498--508 on the blind sets. Those
values drove logits as high as 121.44 and saturated the sigmoid output.

I removed the frequency branch and retrained a semantic-only probe on balanced CIFAKE, SID-Set, and
WildFake data. The resulting model uses 48,000 training rows and passes the recorded gates with
0.991900 SID validation AUC, 0.902925 COCO/DALL-E AUC, and 0.912700 LAION/DALL-E AUC. Its CIFAKE clean
and robust AUCs fall to 0.957820 and 0.896636. The main result is straightforward: robustness to
post-processing and transfer across generators are different objectives. A promotion process needs
to test both, along with score saturation and branch-level feature ranges.

\vspace{5pt}
\noindent\textbf{Keywords:} AI-generated image detection; transformation robustness; domain
generalization; OpenCLIP; frequency analysis; synthetic media forensics.
\end{minipage}
\vspace{0.12in}
]
...
CIFAKE transfers to native-resolution images from unseen sources and generators. \textbf{RQ4} asks
...
\keyfinding{The 0.972310 CIFAKE composite and 0.5000 blind AUC are both correct. They answer different
questions about the same frozen hybrid model.}
...
calibration split. The final head has 512 coefficients and one intercept.
...
fitting. All 20,000 test images are scored under every condition.
...
WildFake default uses COCO val2017 and DALL-E Advanced; a matched configuration uses LAION-5B and
DALL-E Advanced~\cite{wildfake}. Sampling seeds are 2028, 2026, and 2027. The frozen hybrid SHA-256 is
...
\begin{figure*}[!t]
  ...
  images. The x-axis begins at 0.84; labels report absolute AUC.}
...
\subsection{Blind Transfer Reversed the Ranking}

The blind result is the turning point of the project because it overturns the benchmark ranking. The
semantic baseline retains ranking information on all three samples. Both hybrid variants are near
chance, and the selected augmented hybrid calls every image fake at threshold 0.5.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.96\textwidth]{blind_transfer.png}
  \caption{ROC AUC on three frozen, balanced native-resolution samples for the three model variants. Each
  cell uses the same 200-real/200-generated sample.}
  \label{fig:blind}
\end{figure*}

\begin{table*}[t]
\centering
\caption{Blind ROC AUC by frozen model variant. Each cell uses the same 200-real/200-generated sample.}
\label{tab:blind}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
...
LAION-matched configuration, where size-only AUC is 0.5. Resolution mismatch alone therefore does not
...
These values explain why the threshold metrics collapse and why changing the threshold after the fact
would not restore ranking. The semantic branch is not well calibrated by itself: its median real score
on SID is still 0.986973. It does retain order information, but the frequency branch erases that order
by dominating the logit outside the fitting distribution.
...
\begin{table}[H]
\centering
\caption{Promoted semantic-only gates. PFR uses threshold 0.7819586396.}
\label{tab:gates}
\small
\begin{tabularx}{\columnwidth}{@{}Xccc@{}}
\toprule
...
The successor removes the observed saturation and improves held-out ranking. That does not prove
...
underlying dataset. The original blind samples and current gates contain only 400 images each. I used
...
I evaluated transformations individually, while platform pipelines may stack resizing,
recompression, colour adjustment, and sharpening. Scores are not calibrated to deployment prevalence.
...
Blind testing then showed that the selected model was not a general detector. It ranked three
...
useful: transformation robustness and cross-generator transfer must be measured separately, and
...
\path{data/blind-test/ablation_results.json}.
...
\path{outputs/native_training_timing.json}.
